...
def calculate_forces(leg, k_value, *args):
    ...
    for key in args[0][k_value].keys():
        for ax in ['x','y','z']:
            if leg in key and ax in key: force[ax] += sum(f[k_value][key] for f in args if f is not None)
    ...
    for k_value in args[0].keys():
        if constant in k_value:
            ...
            if first_iteration:
                ...
                stack_array = np.vstack((stack_array, data_stack)*scaling_factor)  
...
def calculate_statistics_joints(
    *args,
    ...
    for leg,joint in itertools.product(legs,joints):
        ...
        stack_array = calculate_stack_array(*args, force_calc= force_calculation, \
            leg = name, constant=constant, scaling_factor=scaling_factor)        
...
def calculate_MSE_joints(
    ...
    for gain_name in joint_data.keys():
        for leg in legs:
            ...
            leg_mse.append([leg,float(gain_name[2:5]), float(gain_name[-3:]), mse/len(joints)])
