    ...
    for i, (plot, data) in enumerate(data2plot.items()):
        ...
        if plot == 'torques':
            for joint, torque in data.items():
                ...
                if len(data2plot.keys()) == 1:
                   axs.plot(time[start:stop], torque_adj[start:stop]*torqueScalingFactor,label=joint)
                else:
        ...
        if plot != 'grf' and i == 0:
            ...
            if len(data2plot.keys()) == 1:
                axs.legend(all_handles, all_labels,loc= 'upper right',bbox_to_anchor=(1.135, 1))
            else:
...
def plot_fly_path(begin=0, end=0, sequence=False, save_imgs=False, exp='', heading=True, opt_res=False,generations=[]):
...
def draw_legs_from_2d_data(cams_info,data_2d,exp_dir,side='L',begin=0,end=0,saveimgs = False,pause=0,show_joints=True,dir_name='legsJoints'):
