...
def plot_mu_sem(
    ...
    if len(mu.shape) is 1:
        if conf is not None:
            ax.plot(x[beg:end], mu[beg:end] - conf*error[beg:end], alpha=alpha, \
                linewidth=1.5, linestyle = ':', color='black',\
                label="Confidence Interval {}%".format(conf))
            ax.plot(x[beg:end], mu[beg:end] + conf*error[beg:end], alpha=alpha, \
                linewidth=1.5, linestyle = ':', color='black')
        ax.fill_between(x[beg:end], mu[beg:end] - error[beg:end], mu[beg:end] + error[beg:end], alpha=alpha, color=p[0].get_color())
    else:
        for i in np.arange(mu.shape[1]):
            if conf is not None:
                ax.plot(x[beg:end], mu[beg:end, i]  - conf*error[beg:end, i], alpha=alpha,\
                        linewidth=1.5, linestyle = ':', color='black', \
                        label="Confidence Interval {}%".format(conf))
                ax.plot(x[beg:end], mu[beg:end, i]  + conf*error[beg:end, i], alpha=alpha,\
                        linewidth=1.5, linestyle = ':', color='black')
            ax.fill_between(x[beg:end], mu[beg:end, i] - error[beg:end, i], mu[beg:end, i] + error[beg:end, i],\
...
def plot_kp_joint(
    *args,
    ...
    for k in gain_range:
        ...
        if not calc_force:
            time = np.arange(0, len(args[0][k_value][full_name]), 1)*time_step
            ax.plot(time[beg: beg+intv], np.array(args[0][k_value][full_name][beg:beg+intv])*scaling_factor, color=color, label=k_value)
            ...
            if show_vector:
                for i,axis in enumerate(['x','y','z']):
                    ...
                    ax[i].plot(time[beg: beg+intv], np.array(vector[i,beg:beg+intv])*scaling_factor,\
                ...
                ax.plot(time[beg: beg+intv], norm[beg:beg+intv]*scaling_factor, color=color, label=k_value)
                plt.legend(bbox_to_anchor=(1.1, 1), loc = 'upper right')
