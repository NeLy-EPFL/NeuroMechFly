...
class DrosophilaSimulation(BulletSimulation):
    ...
    def __init__(self, container, sim_options, Kp, Kv, units=SimulationUnitScaling(meters=1000,kilograms=1000)):
        ...
        self.kp = Kp
        self.kv = Kv
        self.claw_track = {'LF':list(), 'LM':list(), 'LH':list(), 'RF':list(), 'RM':list(), 'RH':list()}
        self.angles = self.load_angles('./new_angles/walking_joint_angles_smoothed.pkl') 
        self.velocities = self.load_angles('./new_angles/walking_joint_velocities.pkl') 
    ...
    def controller_to_actuator(self,t):
        ...
        ####LEFT LEGS#######
        vel[self.joint_id['joint_LFCoxa_roll']] = self.velocities['LF_leg']['roll'][ind+0]
        vel[self.joint_id['joint_LFCoxa_yaw']] = self.velocities['LF_leg']['yaw'][ind+0]
        vel[self.joint_id['joint_LFCoxa']] = self.velocities['LF_leg']['pitch'][ind+0]
        vel[self.joint_id['joint_LFFemur_roll']] = self.velocities['LF_leg']['roll_tr'][ind+0]
        vel[self.joint_id['joint_LFFemur']] = self.velocities['LF_leg']['th_fe'][ind+0]
        vel[self.joint_id['joint_LFTibia']] = self.velocities['LF_leg']['th_ti'][ind+0]
        vel[self.joint_id['joint_LFTarsus1']] = self.velocities['LF_leg']['th_ta'][ind+0]

        vel[self.joint_id['joint_LMCoxa_roll']] = self.velocities['LM_leg']['roll'][ind+0]
        vel[self.joint_id['joint_LMCoxa_yaw']] = self.velocities['LM_leg']['yaw'][ind+0]
        vel[self.joint_id['joint_LMCoxa']] = self.velocities['LM_leg']['pitch'][ind+0]
        vel[self.joint_id['joint_LMFemur_roll']] = self.velocities['LM_leg']['roll_tr'][ind+0]
        vel[self.joint_id['joint_LMFemur']] = self.velocities['LM_leg']['th_fe'][ind+0]
        vel[self.joint_id['joint_LMTibia']] = self.velocities['LM_leg']['th_ti'][ind+0]
        vel[self.joint_id['joint_LMTarsus1']] = self.velocities['LM_leg']['th_ta'][ind+0]

        vel[self.joint_id['joint_LHCoxa_roll']] = self.velocities['LH_leg']['roll'][ind+0]
        vel[self.joint_id['joint_LHCoxa_yaw']] = self.velocities['LH_leg']['yaw'][ind+0]
        vel[self.joint_id['joint_LHCoxa']] = self.velocities['LH_leg']['pitch'][ind+0]
        vel[self.joint_id['joint_LHFemur_roll']] = self.velocities['LH_leg']['roll_tr'][ind+0]
        vel[self.joint_id['joint_LHFemur']] = self.velocities['LH_leg']['th_fe'][ind+0]
        vel[self.joint_id['joint_LHTibia']] = self.velocities['LH_leg']['th_ti'][ind+0]
        vel[self.joint_id['joint_LHTarsus1']] = self.velocities['LH_leg']['th_ta'][ind+0]
        
        
        #####RIGHT LEGS######
        vel[self.joint_id['joint_RFCoxa_roll']] = self.velocities['RF_leg']['roll'][ind+0]
        vel[self.joint_id['joint_RFCoxa_yaw']] = self.velocities['RF_leg']['yaw'][ind+0]
        vel[self.joint_id['joint_RFCoxa']] = self.velocities['RF_leg']['pitch'][ind+0]
        vel[self.joint_id['joint_RFFemur_roll']] = self.velocities['RF_leg']['roll_tr'][ind+0]
        vel[self.joint_id['joint_RFFemur']] = self.velocities['RF_leg']['th_fe'][ind+0]
        vel[self.joint_id['joint_RFTibia']] = self.velocities['RF_leg']['th_ti'][ind+0]
        vel[self.joint_id['joint_RFTarsus1']] = self.velocities['RF_leg']['th_ta'][ind+0]

        vel[self.joint_id['joint_RMCoxa_roll']] = self.velocities['RM_leg']['roll'][ind+0]
        vel[self.joint_id['joint_RMCoxa_yaw']] = self.velocities['RM_leg']['yaw'][ind+0]
        vel[self.joint_id['joint_RMCoxa']] = self.velocities['RM_leg']['pitch'][ind+0]
        vel[self.joint_id['joint_RMFemur_roll']] = self.velocities['RM_leg']['roll_tr'][ind+0]
        vel[self.joint_id['joint_RMFemur']] = self.velocities['RM_leg']['th_fe'][ind+0]
        vel[self.joint_id['joint_RMTibia']] = self.velocities['RM_leg']['th_ti'][ind+0]
        vel[self.joint_id['joint_RMTarsus1']] = self.velocities['RM_leg']['th_ta'][ind+0]

        vel[self.joint_id['joint_RHCoxa_roll']] = self.velocities['RH_leg']['roll'][ind+0]
        vel[self.joint_id['joint_RHCoxa_yaw']] = self.velocities['RH_leg']['yaw'][ind+0]
        vel[self.joint_id['joint_RHCoxa']] = self.velocities['RH_leg']['pitch'][ind+0]
        vel[self.joint_id['joint_RHFemur_roll']] = self.velocities['RH_leg']['roll_tr'][ind+0]
        vel[self.joint_id['joint_RHFemur']] = self.velocities['RH_leg']['th_fe'][ind+0]
        vel[self.joint_id['joint_RHTibia']] = self.velocities['RH_leg']['th_ti'][ind+0]
        vel[self.joint_id['joint_RHTarsus1']] = self.velocities['RH_leg']['th_ta'][ind+0]
        

        joint_control_middle = list(np.arange(42,49)) + list(np.arange(81,88))
...
def main():
    ...
    col_hind_leg = [s+'H'+leg for s in side for leg in ['Coxa','Coxa_roll','Femur','Femur_roll','Tibia']]
    col_body_abd = ['prismatic_support_1','prismatic_support_2','A1A2','A3','A4','A5','A6']
    ...
    for Kp in [0.4]:
        for Kv in [0.9]:
            ...
            animal.container.dump(dump_path ="./basepositionrecorded",overwrite=True)
