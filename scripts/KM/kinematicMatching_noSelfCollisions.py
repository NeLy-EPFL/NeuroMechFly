...
class DrosophilaSimulation(BulletSimulation):
    ...
    def calculate_angles(behavior, overwrite_angles=False):
        """Calculate angles"""
        # FIXME: We should restructure the data instead of this
        experiment = data_folder.joinpath(
            behavior, "df3d",
            list(data_folder.glob("pose_result_*.pkl"))[0]
        )
        angles_path = data_folder.joinpath(
            behavior, "df3d",
            list(data_folder.glob("joint_angles_*.pkl"))[0]
        )
...
def save_data(fly, filename):
    ...
    path_torque = script_path.joinpath('results', 'torques_'+filename)
    path_grf = script_path.jointpath('results','grf_'+filename)
    path_ball_rot = script_path.jointpath('results','ballRot_'+filename)
